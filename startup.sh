...
if [ -d "/var/lib/mysql/mysql" ]; then
	...
	do
	  ...
	  sleep 5
        ...
        echo 'mysqld_safe launched.' | tee -a /var/log/startup.log
        mysql -u root < /sw/sql/initdb.sql && echo 'mysql leginondb projecdb created' >> /var/log/startup.log
        mysql -u root leginondb < /sw/sql/leginondb.sql && echo 'mysql leginondb upload' >> /var/log/startup.log
        mysql -u root projectdb < /sw/sql/projectdb.sql && echo 'mysql projectdb upload' >> /var/log/startup.log
